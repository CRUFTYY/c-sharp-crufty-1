...
namespace empleados
{
    internal class Program
    {
        ...
        static void leer(empleados[] a, int total)
        {
            ...
            {
                Console.WriteLine("Ingresar categoriaición del empleado " + ( 1+ i));
                a[i].categoria = Console.ReadLine();
                Console.WriteLine("Ingresar estado del empleado " + ( 1+ i));
                a[i].estado = Console.ReadLine();
                Console.WriteLine("Ingresar salario del empleado " + ( 1+ i));
                a[i].salario = int.Parse(Console.ReadLine());
                Console.WriteLine("Ingresar horas del empleado " + ( 1+ i));
                a[i].horas = int.Parse(Console.ReadLine());
                Console.WriteLine("Ingresar id del empleado " + ( 1+ i));
        ...
        static void aumentogerentes(empleados[] a, int total)
        {
            ...
            for (int i = 1; i < total; i++)
            {
                for (int j = 0; j < total - 1; j++)
                {
                    if (a[j].id < a[j + 1].id)
                    {
                        ...
                        a[j + 1].salario = aux3;
                        aux4 = a[j].categoria;
                        a[j].categoria = a[j + 1].categoria;
                        a[j + 1].categoria = aux4;
                        aux5 = a[j].estado;
            ...
            for (int i = 0; i < total; i++)
                Console.WriteLine("id: " + a[i].id + ", salario: " + a[i].salario + ", horas: " + a[i].horas + ", categoriaición: " + a[i].categoria + ", estado: " + a[i].estado);
        }
        ...
        static void gerentejodon(empleados[] a, int total)
        {
            ...
            for (int i = 0; i < total; i++)
            {
                if (a[i].horas > men && a[i].categoria == "gerente")
                {
                    men = a[i].horas;
                    j = i;
                ...
                Console.WriteLine("id: " + a[j].id + ", salario: " + a[j].salario + ", horas: " + a[j].horas + ", categoriaición: " + a[j].categoria + ", estado: " + a[j].estado);
        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                switch (op)
                {
                    ...
                    default:
                        Console.Clear();
                        Console.WriteLine("Esta opcion es invalida (como vos) ");
                        break;
                }
